...
public class NationalIdValidator implements Validator {
	...
	public static boolean isValidNat(Object value) {
		...
		if (value != null) {
			...
			// "" + Integer.parseInt(value.toString().trim().substring(count -
			// 1, count)) * 2, 2, '0');
			// resultVal = resultVal + Integer.parseInt(("" +
			// paddedVal).substring(0, 1))
			// + Integer.parseInt(("" + paddedVal).substring(1, 2));
			// } else {
			// resultVal = resultVal +
			// Integer.parseInt(value.toString().trim().substring(count - 1,
			// count));
			// }
			// }
			//
			// if (!((((resultVal + "").substring((resultVal + "").length() - 1,
			// (resultVal + "").length())
			// .equals("0")) && (value.toString().trim().substring(9,
			// 10).equals("0")))
			// || ((10 - Integer.parseInt((resultVal + "").substring((resultVal
			// + "").length() - 1,

...
public class ExchangeRequestBean extends ArcScenarioManager {
	...
	public void init() {
		...
		if (arcRecordId != null) {
			...
			System.out.println("user manger id : " + dataAccessService.getUserById(request.getUserId()).getMgrId());
			System.out.println("step Number : " + stepNum);
			if ((dataAccessService.getUserById(request.getUserId()).getMgrId()).equals(storeDeanId)) {
				if (mgrId.equals(currentUser.getUserId()) && 0 < stepNum && stepNum <= 2) {
					enableAccept = true;
					enableRefuse = true;
				}
	...
	public String printExchangeReportAction() {
		...
		if (whsWarehouses.getStrType() == 2) { /// طلب رجيع
			...
			Utils.printPdfReport(reportName, parameters);
			System.out.println(request.getArcUser().getEmployeeName());
			System.out.println(request.getGeneralrequestNumber());
		}
		return "";
	}
	...
	public String printArticalCardReportAction() {
		...
		for (StoreRequestModel requestdets : artHistory) {
			...
			if (requestdets.getSerialNumber() != null)
	...
	public boolean cheackQtyDetailsOfRequst(Integer qty, Integer artId, Integer storeId) {
		StoreRequestModel storeRequestModelQty = new StoreRequestModel();
		List<StoreRequestModel> storeRequestModelQtyList = new ArrayList<StoreRequestModel>();
		storeRequestModelQtyList = dataAccessService.getTransactionsQty(artId, storeId);
		if (storeRequestModelQtyList.isEmpty()) {
			MsgEntry.addErrorMessage(" هذا الصنف ليس به رصيد");
			return false;
		} else {
			storeRequestModelQty = storeRequestModelQtyList.get(0);

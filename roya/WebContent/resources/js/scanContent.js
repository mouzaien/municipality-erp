...
Dynamsoft.WebTwainEnv.RegisterEvent('OnWebTwainReady', Dynamsoft_OnReady); // Register
																			// OnWebTwainReady
																			// event.
																			// This
																			// event
...
var DWObject, blankField = "", extrFieldsCount = 0, upload_returnSth = true, strHTTPServer = location.hostname, strActionPage, strFullActionPagePath, ObjString, CurrentPathName = unescape(location.pathname), CurrentPath = CurrentPathName
		.substring(0, CurrentPathName.lastIndexOf("/") + 1);
...
function languageSelected() {
	...
	if ([ 0, 2, 4, 6 ]
			.indexOf(document.getElementById("ddlLanguages").selectedIndex) == -1) {
...
function addAField() {
	...
	if (document.getElementById('div-extra-fields').style.display == "none")
		...
		blankField = document.getElementsByClassName('div-fields-item')[extrFieldsCount - 1]
				.cloneNode(true);
	}
}
...
function downloadPDFR() {
	...
	DWObject.Addon.PDF.Download(strPDFPath, function() {
...
function Dynamsoft_OnReady() {
	blankField = document.getElementsByClassName('div-fields-item')[0]
			.cloneNode(true);
	...
	if (DWObject) {
		...
		// document.getElementById("ddlLanguages").options.add(new
		// Option(scriptLanguages[i].desc, i));
		...
		if (DWObject.Addon.PDF.IsModuleInstalled()) {
			...
			ObjString
					.push('The <strong>PDF Rasterizer</strong> is not installed on this PC. Please click the button below to get it installed');
			ObjString
					.push('<p class="tc mt15 mb15"><input type="button" value="Install PDF Rasterizer" onclick="downloadPDFR();" class="btn lgBtn bgBlue" /><hr></p>');
			ObjString
					.push('<i><strong>The installation is a one-time process</strong> <br />It might take some time depending on your network.</i>');
			ObjString.push('</div>');
...
function _printUploadedFiles(info) {
	...
	if (info.indexOf('DWTUploadFileName') != -1) {
		...
		if (info.indexOf("DWTUploadFileIndex:") != -1) {
			savedIntoToDB = true;
			imgIndexInDB = info.substring(
					info.indexOf('DWTUploadFileIndex') + 19, info
							.indexOf('DWTUploadFileName'));
			// console.log(imgIndexInDB);
		}
		var fileName = info.substring(info.indexOf('DWTUploadFileName') + 18,
				info.indexOf('UploadedFileSize'));
		var fileSize = info.substr(info.indexOf('UploadedFileSize') + 17);
		if (savedIntoToDB) {
			if (info.indexOf('CSHARP') != -1) {
				url += CurrentPath + 'action/csharp-db.aspx?imgID='
						+ imgIndexInDB;
			} else if (info.indexOf('VBNET') != -1) {
				url += CurrentPath + 'action/vbnet-db.aspx?imgID='
						+ imgIndexInDB;
			} else if (info.indexOf('PHP') != -1) {
				url += CurrentPath + 'action/php-mysql.php?imgID='
						+ imgIndexInDB;
			} else if (info.indexOf('JSP') != -1) {
				url += CurrentPath + 'action/jsp-oracle.jsp?imgID='
						+ imgIndexInDB;
			}
		} else {
			url += CurrentPath + 'action/Dynamsoft_Upload/'
					+ encodeURI(fileName);
		}
		var newTR = document.createElement('tr');
		_str = "<td class='tc'><a class='bluelink'" + ' href="' + url
				+ '" target="_blank">' + fileName + "</a></td>"
				+ "<td class='tc'>" + fileSize + '</td>';
		if (info.indexOf("FieldsTrue:") != -1)
			_str += "<td class='tc'><a class='bluelink'" + '" href="'
					+ url.substring(0, url.length - 4) + '_1.txt'
					+ '" target="_blank">Fields</td>';
		else {
...
function upload_preparation(_name) {
	...
	if (String.isNullOrEmpty(document.getElementById('includeform:mylst').value)) {
		document.getElementById('includeform:mylst').value = _name;
	} else {
		document.getElementById('includeform:mylst').value = document
		.getElementById('includeform:mylst').value+ ";"
		+ _name ;
	}
	...
	if (Dynamsoft.Lib.detect.ssl == true) {
		_strPort = location.port == "" ? 443 : location.port;
		strFullActionPagePath = "https://" + strHTTPServer + ":" + _strPort
				+ strActionPage;
	} else {
		strFullActionPagePath = "http://" + strHTTPServer + ":" + _strPort
				+ strActionPage;
	}

	DWObject.HTTPPort = _strPort;
	// if ([0, 2, 4,
	// 6].indexOf(document.getElementById("ddlLanguages").selectedIndex) != -1)
	// {
...
function UploadImage_inner() {
	...
	var i, aryIndices = [], Digital = new Date(), uploadfilename = Digital
			.getTime();
	upload_preparation(uploadfilename);
	// Upload the image(s) to the server asynchronously
	// if (document.getElementById("ddlLanguages").options.selectedIndex == 11
	// /*Azure*/) return;
	...
	// if (upload_returnSth)
	// DWObject.HTTPUpload(strFullActionPagePath, aryIndices.splice(0, 1),
	// EnumDWT_ImageType.IT_JPG, EnumDWT_UploadDataFormat.Binary, uploadfilename
	// + "-" + (count - aryIndices.length) + ".jpg", OnHttpUploadSuccess,
	// uploadJPGsOneByOne);
	// else
	// DWObject.HTTPUpload(strFullActionPagePath, aryIndices.splice(0, 1),
	// EnumDWT_ImageType.IT_JPG, EnumDWT_UploadDataFormat.Binary, uploadfilename
	// + "-" + (count - aryIndices.length) + ".jpg", uploadJPGsOneByOne,
	...
	// DWObject.HTTPUpload(strFullActionPagePath, aryIndices,
	// EnumDWT_ImageType.IT_TIF, EnumDWT_UploadDataFormat.Binary, uploadfilename
	// + ".tif", OnHttpUploadSuccess, OnHttpServerReturnedSomething);
	// }
	if (document.getElementsByName('ImageType')[0].checked) {
		aryIndices = [];
		...
		DWObject.HTTPUpload(strFullActionPagePath, aryIndices,
				...
				uploadfilename + ".pdf", OnHttpUploadSuccess,
				OnHttpServerReturnedSomething);
	}
}
...
function UploadImage() {
	...
	if (DWObject) {
		...
		// var ds_load_file_to_upload_directly = function (bSave, filesCount,
		// index, path, filename) {
		// nCount = filesCount;
		...
		// DWObject.HTTPUploadThroughPostDirectly(strHTTPServer, filePath,
		// strActionPage, filename,
		// function () {
		...
		// uploadFileOneByOne();
		// else
		// DWObject.UnregisterEvent('OnGetFilePath',
		// ds_load_file_to_upload_directly);
		// },
		// OnHttpServerReturnedSomething
		// );
		// };
		...
		// ds_load_file_to_upload_directly);
		// DWObject.ShowFileDialog(false, "Any File | *.*", 0, "", "", true,
		// true, 0);
		// }
		// else {
		UploadImage_inner();
...
function preparetoUploadtoAzure(__name) {
	...
	var actionPageFullPath = CurrentPath + 'action/' + 'azure.aspx?imageName='
			+ uploadfilename;
